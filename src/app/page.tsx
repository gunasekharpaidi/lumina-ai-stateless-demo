    ...
    return (
        <div className="absolute inset-0 bg-white/95 z-[60] flex flex-col items-center justify-center p-10 animate-in fade-in duration-500 font-sans">
            <div className="relative group shadow-2xl rounded-[40px] overflow-hidden bg-white border border-zinc-200" style={{ aspectRatio: '3/4', maxHeight: '70vh' }}>
                ...
                <div className="absolute top-6 left-6 z-20 flex items-center gap-2 px-4 py-2 bg-white/70 rounded-xl border border-white/50 backdrop-blur-md shadow-premium">
                    <Camera className="w-3.5 h-3.5 text-indigo-600" />
                    <span className="text-[10px] font-black text-zinc-950 uppercase tracking-[.2em]">Studio: {shotType}</span>
                </div>
                
                {isProcessing && (
                    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/40 backdrop-blur-xl animate-in fade-in duration-500">
                        <div className="relative mb-8">
                            <div className="w-24 h-24 rounded-full border-2 border-indigo-100 flex items-center justify-center">
                                <Loader2 className="w-8 h-8 text-indigo-600 animate-spin" />
                            </div>
                            <div className="absolute inset-0 w-24 h-24 rounded-full border-t-2 border-indigo-600 animate-spin duration-1000"></div>
                        </div>
                        <p className="text-[11px] font-black tracking-[.6em] text-zinc-950 uppercase animate-pulse">Syncing Synthesis...</p>
                        <p className="text-[9px] font-bold text-premium-muted uppercase mt-5 tracking-[.3em]">Applying Pixel-Perfect Retouch</p>
                    </div>
                )}
            </div>

            <div className={`mt-10 w-full max-w-3xl bg-white border border-zinc-100 rounded-[40px] p-10 flex flex-col gap-8 shadow-elite transition-all duration-700 ${isProcessing ? 'opacity-40 pointer-events-none scale-95 blur-sm' : ''}`}>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-5">
                        <div className="w-12 h-12 rounded-full border border-zinc-100 p-0.5 bg-white shadow-premium ring-4 ring-zinc-50/50 flex items-center justify-center">
                            <User className="w-6 h-6 text-zinc-400" />
                        </div>
                        <div className="flex bg-zinc-50 rounded-2xl p-1.5 border border-zinc-100 shadow-inner">
                            <div className="flex items-center gap-2 bg-white rounded-xl px-4 py-2 border border-indigo-100 shadow-sm">
                                <Paintbrush className="w-4 h-4 text-indigo-600" />
                                <span className="text-[11px] font-black text-indigo-600 uppercase tracking-widest">Paint</span>
                            </div>
                            <button onClick={setupCanvas} className="flex items-center gap-2 px-4 py-2 rounded-xl text-premium-muted hover:text-zinc-950 transition-all group" title="Clear Mask">
                        ...
                        <div className="flex items-center gap-4 ml-2">
                            <span className="text-[10px] font-black text-zinc-400 uppercase tracking-[.2em]">Size</span>
                            <input 
...
export default function Dashboard() {
  ...
  // Helper: non-authenticated fetch to Railway backend
  ...
  return (
    <div className="flex h-screen w-full bg-[#fdfdfd] overflow-hidden font-sans text-zinc-600">
      ...
      <aside className="w-[340px] border-r border-zinc-100 bg-white flex flex-col pt-12 pb-8 px-10 z-30 shadow-[4px_0_32px_rgba(0,0,0,0.01)] overflow-y-auto no-scrollbar relative">
        <div className="absolute inset-0 bg-gradient-to-b from-zinc-50/20 to-transparent pointer-events-none" />
        <div className="flex items-center gap-4 w-full mb-12 relative z-10">
            <div className="h-14 w-14 rounded-[20px] bg-zinc-950 flex items-center justify-center shadow-xl shadow-zinc-200 border border-zinc-900 group hover:scale-105 transition-all duration-500">
                <Sparkles className="text-white w-7 h-7" />
            </div>
            <div>
                <h1 className="text-2xl font-black tracking-tighter text-zinc-950 leading-none">Lumina<span className="text-indigo-600 block text-xs tracking-[.4em] mt-2 non-italic">PRO STUDIO</span></h1>
            </div>
        </div>
        ...
        <div className="flex-1 space-y-12 w-full pb-10">
             ...
             <div className="grid grid-cols-2 gap-3">
                {environments.map(env => (
                    <button
                        ...
                        className={`flex flex-col items-center gap-3 p-5 rounded-[28px] border transition-all duration-300 relative group ${
                            environment === env.id 
                                ? 'bg-zinc-950 text-white border-zinc-900 shadow-xl scale-[1.05] z-10' 
                                : 'bg-white border-zinc-100 text-zinc-400 hover:text-indigo-600 hover:border-indigo-100 shadow-premium'
                        }`}
                    >
                        <env.icon className={`w-5 h-5 transition-transform group-hover:scale-110 ${environment === env.id ? 'text-indigo-400' : ''}`} />
                        <span className="text-[10px] font-black uppercase tracking-widest">{env.id}</span>
                    </button>
                ))}
             </div>
          </div>
          ...
          <div className={`space-y-4 pt-8 border-t border-white/5 transition-all duration-700 ${apparelMode === 'laydown' ? 'opacity-20 pointer-events-none grayscale' : ''}`}>
            ...
            {genders.map((g) => (
                ...
                className={`px-3 py-3 text-[11px] font-black rounded-xl border transition-all uppercase tracking-[.15em] ${
                    modelConfig.gender === g 
                    ? 'bg-zinc-950 text-white border-zinc-900 shadow-lg' 
                    : 'bg-white border-zinc-100 text-premium-muted hover:bg-zinc-50 hover:text-zinc-900 transition-colors'
                }`}
                >
                {g}
                </button>
          ...
          <div className="space-y-4 border-t border-white/5 pt-8">
            ...
            <textarea 
              ...
              className="w-full bg-zinc-50 border border-zinc-100 rounded-[24px] text-[11px] text-zinc-900 p-6 min-h-[120px] focus:outline-none focus:border-indigo-500/30 placeholder:text-premium-muted font-medium transition-all shadow-inner"
            />
          </div>
        </div>
      ...
      <main className="flex-1 bg-[#fdfdfd] relative flex flex-col overflow-hidden">
        
        <header className="h-[88px] border-b border-zinc-100 flex items-center justify-between px-16 relative z-40 bg-white/50 backdrop-blur-xl">
           <div className="flex items-center gap-1.5 p-1.5 rounded-[24px] bg-zinc-50 border border-zinc-100/50 shadow-inner">
                {studios.map(studio => (
                  <button
                    key={studio.id} 
                    onClick={() => { 
                    ...
                    className={`flex items-center gap-4 px-6 py-3 rounded-2xl transition-all duration-500 relative group/sbtn ${
                        studio.comingSoon
                            ? 'text-zinc-300 cursor-not-allowed grayscale opacity-50'
                            : activeStudio === studio.id 
                                ? 'bg-white text-zinc-950 shadow-elite border border-zinc-100 scale-[1.02]' 
                                : 'text-premium-muted hover:text-zinc-950'
                    }`}
                  >
                    <studio.icon className={`w-4 h-4 transition-colors ${activeStudio === studio.id ? 'text-indigo-600' : 'group-hover/sbtn:text-indigo-600'}`} />
              ...
              <a href="/pricing" className="flex items-center gap-4 px-5 py-2 rounded-2xl bg-white border border-zinc-200 hover:border-indigo-300 transition-all group shadow-sm">
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2 py-0.5 rounded-md text-[7px] font-black uppercase tracking-[0.2em] ${
                      userPlan === "PRO" ? "bg-indigo-600 text-white shadow-lg shadow-indigo-200" :
                      userPlan === "STARTER" ? "bg-indigo-100 text-indigo-600" :
                      "bg-zinc-100 text-premium-muted"
                    }`}>
                      {userPlan}
                    </span>
                  </div>
